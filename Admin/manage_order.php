...
<style>
    ...
    /* Nicer View button for Manage Orders */
...
<body class="editor-modern compact">
    <div class="container">
        ...
        <div class="main-content admin-dashboard">
            ...
            $filterStatus = strtolower(trim($_GET['status'] ?? 'completed'));
                ...
                <div class="panel-head">
                    ...
                    <div class="panel-actions">
                        <div class="search" style="min-width:260px">
                            <i class="fas fa-search"></i>
                            <input id="tableFilter" type="text" placeholder="Filter by customer, product, color…" />
                        </div>
                    </div>
                </div>
                <nav class="status-chips" aria-label="Filter by status">
                    <a class="chipx comp <?php echo ($filterStatus === 'completed') ? 'active' : ''; ?>"
                        href="manage_order.php?status=completed">Completed <span
                            class="badge"><?php echo $counts['completed']; ?></span></a>
                    <a class="chipx proc" href="orders.php?status=processing">Processing <span
                            class="badge"><?php echo $counts['processing']; ?></span></a>
                    <a class="chipx pack" href="orders.php?status=packed">Packed <span
                            class="badge"><?php echo $counts['packed']; ?></span></a>
                    <a class="chipx ship" href="orders.php?status=shipped">Shipped <span
                            class="badge"><?php echo $counts['shipped']; ?></span></a>
                    <a class="chipx delv" href="orders.php?status=delivered">Delivered <span
                            class="badge"><?php echo $counts['delivered']; ?></span></a>
                    <a class="chipx canc" href="orders.php?status=cancelled">Cancelled <span
                    ...
                    WHERE $where
                            ...
                            if ($result->num_rows > 0) {
                                while ($row = $result->fetch_assoc()) {
                                    ...
                                    echo "<input type='hidden' name='from' value='manage_order'>";
                                    if ($isValid) {
                                    ...
                                    echo "</td>";
                                    echo "<td>";
                                    echo "<button class='btn btn-secondary js-view' data-id='" . (int) $row['order_id'] . "' title='View details'><i class='fas fa-eye' style='margin-right:6px'></i>View</button>";
                                    echo "</td>";
                                    echo "</tr>";
                                }
                            } else {
                                echo "<tr><td colspan='11'>No completed orders.</td></tr>";
                            }
                            ?>
                        </tbody>
                    </table>
    ...
    <script>
        ...
        (function () {
            ...
            var COMPANY = { name: 'Apple Care+', logo: '/Apple_Care/Assets/Images/apple.png', address: 'No. 123, Main Street, Colombo, Sri Lanka', phone: '+94 11 123 4567', email: 'support@applecare.local', socials: [{ icon: 'fab fa-facebook', label: '@applecare' }, { icon: 'fab fa-instagram', label: '@applecare' }, { icon: 'fab fa-twitter', label: '@applecare' }, { icon: 'fab fa-linkedin', label: 'Apple Care+' }] };
            function buildQuotationHTML(d) {
                var unit = (Number(d.total_amount || 0) / Math.max(1, Number(d.total_quantity || 1))); var custLoc = [d.city, d.state, d.zip_code].filter(Boolean).join(', '); var od = new Date(d.order_date || Date.now()); var valid = new Date(od.getTime()); valid.setDate(valid.getDate() + 14); function fmtDate(dt) { try { return dt.toISOString().slice(0, 10); } catch (e) { return String(d.order_date || ''); } }
                return `<!DOCTYPE html><html><head><meta charset="utf-8"><title>${h(COMPANY.name)} - Quotation</title><link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=Nunito:wght@400;600;800&display=swap" rel="stylesheet"><link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.4/css/all.min.css"><style>:root{ --brand:#111827; --accent:#0ea5e9; --accent2:#22c55e; --muted:#374151; --border:#e5e7eb; } *{box-sizing:border-box} body{font-family:'Nunito', Arial, Helvetica, sans-serif; color:var(--brand); margin:0;} .brandbar{height:8px; background:linear-gradient(90deg,var(--accent),var(--accent2));} .container{padding:24px;} .header-split{position:relative; padding-left:96px; text-align:center;} .logo-left{position:absolute; left:0; top:0; width:72px; height:72px; object-fit:contain} .title{margin:0; font-size:22px; letter-spacing:-.01em} .meta{font-size:12px; color:var(--muted)} .socials{margin-top:6px; display:flex; gap:12px; color:var(--brand); font-size:12px; justify-content:center} .socials i{margin-right:6px} .note{color:var(--muted); font-size:12px; margin-top:4px} .underline{height:2px; background:linear-gradient(90deg,rgba(0,0,0,.15),transparent); margin:14px 0 18px} .sec{margin-bottom:16px} .grid{display:grid; grid-template-columns: 1.1fr 1fr; gap:16px} .block{border:1px solid var(--border); border-radius:12px; padding:14px} .product{display:grid; grid-template-columns: 140px 1fr; gap:12px; align-items:start} .product img{width:100%; max-height:140px; object-fit:contain; border:1px solid var(--border); border-radius:10px; background:#fff} .tags{display:flex; gap:8px; margin-top:6px} .tag{font-size:12px; padding:4px 8px; border-radius:999px; border:1px solid var(--border); background:#f8fafc} .tbl{width:100%; border-collapse:collapse; margin-top:10px} .tbl thead th{background:#f1f5f9} .tbl th,.tbl td{border:1px solid var(--border); padding:8px; text-align:left} .tbl tbody tr:nth-child(odd){background:#fafafa} .totals{display:flex; justify-content:flex-end} .totals .pill{padding:10px 14px; border-radius:12px; background:#ecfeff; border:1px solid #a5f3fc; font-weight:800} .terms{font-size:12px; color:var(--muted)} .signs{display:grid; grid-template-columns: 1fr 1fr; gap:20px} .sign{height:90px} .sign .line{margin-top:46px; border-top:1px solid #6b7280; width:70%} .footer{position:fixed; bottom:0; left:0; right:0; padding:8px 24px; font-size:12px; color:#374151; display:flex; justify-content:space-between; border-top:1px solid var(--border)} .footer .pages:after{content: counter(page) ' / ' counter(pages);} @media print { @page{ margin:14mm; } body{margin:0;} .container{padding:16px 0;} }</style></head><body><div class="brandbar"></div><div class="container"><div class="header-split"><img class="logo-left" src="${h(COMPANY.logo)}" alt="${h(COMPANY.name)}"><div class="centered"><h1 class="title">${h(COMPANY.name)}</h1><div class="meta">${h(COMPANY.address)} · ${h(COMPANY.phone)} · ${h(COMPANY.email)}</div><div class="socials">${COMPANY.socials.map(s => `<span><i class='${s.icon}'></i>${h(s.label)}</span>`).join('')}</div><div class="note">Order #${h(d.order_id)} · Date ${h(d.order_date)} · Valid until ${fmtDate(valid)}</div></div></div><div class="underline"></div><div class="sec grid"><div class="block"><h3 style="margin:0 0 8px">Product</h3><div class="product"><img src="${h(d.image_url)}" alt="${h(d.product_name)}"><div><div style="font-weight:800; font-size:16px">${h(d.product_name)}</div><div class="tags"><span class="tag"><i class="fas fa-palette"></i> ${h(d.item_color)}</span><span class="tag"><i class="fas fa-hdd"></i> ${h(d.storage)}</span></div><table class="tbl"><thead><tr><th>Qty</th><th>Unit Price</th><th>Total</th></tr></thead><tbody><tr><td>${h(d.total_quantity)}</td><td>Rs${unit.toFixed(2)}</td><td>Rs${Number(d.total_amount || 0).toFixed(2)}</td></tr></tbody></table></div></div></div><div class="block"><h3 style="margin:0 0 8px">Customer</h3><div style="font-weight:800">${h(d.first_name + ' ' + d.last_name)}</div><div>${h(d.address || '')}</div><div>${h(custLoc)}</div><div>${h(d.country || '')}</div><div style="margin-top:6px">Phone: ${h(d.phone_number || '')} · Email: ${h(d.email || '')}</div></div></div><div class="sec totals"><span class="pill">Grand Total: Rs${Number(d.total_amount || 0).toFixed(2)}</span></div><div class="sec"><div class="block terms"><strong>Terms & Conditions</strong><ul><li>Quotation valid until ${fmtDate(valid)}.</li><li>Prices include applicable taxes unless otherwise specified.</li><li>Payment due prior to delivery; bank transfer or card accepted.</li><li>Warranty as per manufacturer policy.</li></ul></div></div><div class="sec signs"><div class="sign"><div>Customer Signature</div><div class="line"></div></div><div class="sign"><div>Authorized Signature</div><div class="line"></div></div></div></div><div class="footer"><span>${h(COMPANY.name)}</span><span class="pages"></span></div></body></html>`;
            }
            function printQuotation() { if (!currentOrder) return; var html = buildQuotationHTML(currentOrder); var w = window.open('', '_blank'); if (!w) { return; } w.document.open(); w.document.write(html); w.document.close(); w.focus(); setTimeout(function () { try { w.print(); } catch (e) { } }, 400); }
            function buildGroupQuotationHTML(payload) { var c = payload.customer; var items = payload.items || []; var grand = Number(payload.grand_total || 0); var from = payload.group.from_date, to = payload.group.to_date; var validText = ''; try { var vd = new Date(to || Date.now()); vd.setDate(vd.getDate() + 14); validText = vd.toISOString().slice(0, 10); } catch (e) { validText = String(to || ''); } function row(i) { return `<tr><td>${i.product_name}</td><td>${i.item_color}</td><td>${i.storage}</td><td>${i.total_quantity}</td><td>Rs${Number(i.unit_price || 0).toFixed(2)}</td><td>Rs${Number(i.total_amount || 0).toFixed(2)}</td></tr>`; } return `<!DOCTYPE html><html><head><meta charset='utf-8'><title>${h(COMPANY.name)} - Quotation</title><link rel='preconnect' href='https://fonts.googleapis.com'><link rel='preconnect' href='https://fonts.gstatic.com' crossorigin><link href='https://fonts.googleapis.com/css2?family=Nunito:wght@400;600;800&display=swap' rel='stylesheet'><link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.4/css/all.min.css"><style>:root{ --brand:#111827; --accent:#0ea5e9; --accent2:#22c55e; --muted:#374151; --border:#e5e7eb; } *{box-sizing:border-box} body{font-family:'Nunito', Arial, Helvetica, sans-serif; color:var(--brand); margin:0;} .brandbar{height:8px; background:linear-gradient(90deg,var(--accent),var(--accent2));} .container{padding:24px;} .header-split{position:relative; padding-left:96px; text-align:center;} .logo-left{position:absolute; left:0; top:0; width:72px; height:72px; object-fit:contain} .title{margin:0; font-size:22px; letter-spacing:-.01em} .meta{font-size:12px; color:var(--muted)} .socials{margin-top:6px; display:flex; gap:12px; color:var(--brand); font-size:12px; justify-content:center} .socials i{margin-right:6px} .note{color:var(--muted); font-size:12px; margin-top:4px} .underline{height:2px; background:linear-gradient(90deg,rgba(0,0,0,.15),transparent); margin:14px 0 18px} .tbl{width:100%; border-collapse:collapse; margin-top:10px} .tbl thead th{background:#f1f5f9} .tbl th,.tbl td{border:1px solid var(--border); padding:8px; text-align:left} .tbl tbody tr:nth-child(odd){background:#fafafa} .totals{text-align:right; font-weight:800; margin-top:10px} .pill{display:inline-block; padding:10px 14px; border-radius:12px; background:#ecfeff; border:1px solid #a5f3fc; font-weight:800} .terms{font-size:12px; color:var(--muted)} .signs{display:grid; grid-template-columns: 1fr 1fr; gap:20px; margin-top:10px} .sign{height:90px} .sign .line{margin-top:46px; border-top:1px solid #6b7280; width:70%} .footer{position:fixed; bottom:0; left:0; right:0; padding:8px 24px; font-size:12px; color:#374151; display:flex; justify-content:space-between; border-top:1px solid var(--border)} .footer .pages:after{content: counter(page) ' / ' counter(pages);} @media print { @page{ margin:14mm; } body{margin:0;} .container{padding:16px 0;} }</style></head><body><div class='brandbar'></div><div class='container'><div class='header-split'><img class='logo-left' src='${h(COMPANY.logo)}' alt='${h(COMPANY.name)}'><div class='centered'><h1 class='title'>${h(COMPANY.name)}</h1><div class='meta'>${h(COMPANY.address)} · ${h(COMPANY.phone)} · ${h(COMPANY.email)}</div><div class='socials'>${COMPANY.socials.map(s => `<span><i class='${s.icon}'></i>${h(s.label)}</span>`).join('')}</div><div class='note'>Orders between ${h(from)} and ${h(to)} · Valid until ${h(validText)}</div></div></div><div class='underline'></div><div><strong>Customer:</strong> ${h(c.first_name + ' ' + c.last_name)} · ${h(c.address || '')} · ${h([c.city, c.state, c.zip_code].filter(Boolean).join(', '))} · ${h(c.country || '')} · Phone: ${h(c.phone_number || '')} · Email: ${h(c.email || '')}</div><table class='tbl'><thead><tr><th>Product</th><th>Color</th><th>Storage</th><th>Qty</th><th>Unit Price</th><th>Total</th></tr></thead><tbody>${items.map(row).join('')}</tbody></table><div class='totals'><span class='pill'>Grand Total: Rs${grand.toFixed(2)}</span></div><div class='terms'><strong>Terms & Conditions</strong><ul><li>Quotation valid until ${h(validText)}.</li><li>Prices include applicable taxes unless otherwise specified.</li><li>Payment due prior to delivery; bank transfer or card accepted.</li><li>Warranty as per manufacturer policy.</li></ul></div><div class='signs'><div class='sign'><div>Customer Signature</div><div class='line'></div></div><div class='sign'><div>Authorized Signature</div><div class='line'></div></div></div></div><div class='footer'><span>${h(COMPANY.name)}</span><span class='pages'></span></div></body></html>`; }
